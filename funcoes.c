...
#define LIMITE_TEMPO_PEDIDO 300 // 5 minutos em segundos
#define MIN_ESTOQUE_PROATIVO 2 // Nível de estoque desejado
#define MAX_PEDIDOS_EM_PRODUCAO 5 // Limite de pedidos na cozinha (N)

static int next_pedido_id = 1; // ID global para todos os pedidos
...
NodePedido* removerPedidoPorPrioridade(Locais *local, const char** origem_fila) {
    // 1. Checa Heap (Prioridade Máxima)
    ...
    // 2. Checa Fila de Espera por iFood (Primeiro iFood encontrado)
    ...
    // 3. Se não há iFood, pega o primeiro Presencial
...
void montarBandeja(Restaurante *res) {
    ...
    while (local_montagem->heap.quantidade > 0 || local_montagem->fila_espera.cabeca != NULL) {
        ...
        if (montador == NULL) break; // Sem funcionários
...
void inicializarFuncionarios(Restaurante *res) {
    ...
    // 2 funcionários habilitados a fazer as batatas fritas
    ...
    // 2 funcionários habilitados a trabalhar no caixa
...
NodeFuncionario* encontrarEAlocarFuncionario(ListaFuncionarios *reserva, Habilidade habilidade) {
    ...
    return NULL; // Ninguém foi encontrado
}
...
void liberarFuncionario(NodeFuncionario *node, ListaFuncionarios *origem, ListaFuncionarios *destino_reserva) {
    ...
    // O print foi silenciado para reduzir o ruído
...
float estimarTempoTotalPedido(Pedido* pedido, Restaurante *res) {
    ...
    // A estimativa agora começa a contar da fila do separador (cozinha)
    ...
    tempo_estimado += 25; // Tempo de montagem
...
static void gerenciarEquipamentoEspecifico(Equipamento *equip, Habilidade habilidade_necessaria, ListaFuncionarios *reserva, ListaItensArmazenados *estoque, bool restaurante_aberto, int restaurante_id) {
    ...
    // --- PARTE 1: ATENDER PEDIDOS DE CLIENTES (DA FILA) ---
    while (equip->capacidade_usada < equip->capacidade_maxima && equip->fila_espera.cabeca != NULL) {
        ...
        if (slot_vazio != -1) {
            ...
            adicionarListaItemPreparo(&equip->fila_espera, item_node); // Devolve pra fila
            liberarFuncionario(operador, NULL, reserva);
            break; // Slots cheios
        }
    } // Fim da Parte 1

    // --- PARTE 2: PRODUÇÃO PROATIVA PARA ESTOQUE ---
    ...
    // Contar estoque atual + em preparo
    int estoque_atual = 0;
...
// Assinaturas das funções wrapper atualizadas
...
// Função auxiliar para adicionar múltiplos itens de um tipo ao pedido
...
NodePedido* lerNovoPedidoFormatado(FILE *arquivo_entrada, int tempo_simulacao_atual) {
    ...
    // 1. Tipo (Ifood/Presencial)
    if (!fgets(buffer, sizeof(buffer), arquivo_entrada)) {
        printf("[SISTEMA] Fim do arquivo de entrada.\n");
        next_pedido_id--; 
        return NULL; // Retorna NULL se o arquivo acabou
    }
    sscanf(buffer, "%s", tipo_pedido_str); 

    // 2. Senha
    if (!fgets(buffer, sizeof(buffer), arquivo_entrada)) { return NULL; } // EOF
    sscanf(buffer, "%s", senha_str);

    // 3. <<< LÓGICA DE PAUSA >>>
    if (strcmp(senha_str, "0") == 0 || strcmp(senha_str, "-") == 0) {
        p.id_senha = 0; // Sinal de Pausa
        // Pula as 14 linhas restantes
        for (int i = 0; i < 14; i++) {
            if (!fgets(buffer, sizeof(buffer), arquivo_entrada)) {
                return NULL; // EOF Inesperado
            }
        }
    ...
    // Se não for pausa, continua lendo normalmente
    ...
    if (soma >= LIMITE_TEMPO_PEDIDO) {
        ...
        printf("----------------------------------------------------------------\n");
        next_pedido_id--; 
        return NULL; // Retorna NULL (pedido rejeitado)     
    }
...
// <<< ESTA É A NOVA FUNÇÃO ROTEADORA (Regra 1 e 3) >>>
bool recepcao(FILE *arquivo_entrada, Restaurante restaurantes[], int timer_global, int tempo_simulacao_atual, bool restaurante_aberto) {
    ...
    if (novo_pedido->pedido.id_senha == 0) {
        ...
        free(novo_pedido); 
        return true; // Sinaliza para CONTINUAR lendo (mas sem processar pedido)
    }

    // 3. ROTEAMENTO (REGRA 3: iFood por distância)
    ...
    if (d1 == 0) { // Presencial R1
        restaurante_alvo = &restaurantes[0];
        novo_pedido->pedido.tipo = PRESENCIAL;
    } else if (d2 == 0) { // Presencial R2
        restaurante_alvo = &restaurantes[1];
        novo_pedido->pedido.tipo = PRESENCIAL;
    } else { // iFood
        novo_pedido->pedido.tipo = IFOOD;
        if (d1 < d2) {
            restaurante_alvo = &restaurantes[0]; // iFood para R1
        } else if (d2 < d1) {
            restaurante_alvo = &restaurantes[1]; // iFood para R2
        } else {
            // Desempate por senha
    ...
    // 4. Restaurante está aberto? (VERIFICAÇÃO DO BUG DO LOOP INFINITO)
    if (!restaurante_aberto) {
        printf("[ROTEADOR] Restaurante fechado. Pedido %d (Senha %d) rejeitado.\n",
               novo_pedido->pedido.id, novo_pedido->pedido.id_senha);
        free(novo_pedido);
        return true; // Rejeitado, mas continue lendo o arquivo
    }
    
    // 5. VERIFICAÇÃO DE CAIXA (REGRA 1)
    ...
    if (caixa == NULL) {
        ...
        free(novo_pedido);
        return true; // Pedido descartado, mas continue lendo
    }

    // 6. Checagem N=5 (Bloqueio da cozinha)
    ...
    if (total_em_producao >= MAX_PEDIDOS_EM_PRODUCAO) {
        ...
        liberarFuncionario(caixa, NULL, &restaurante_alvo->reserva); // Libera o caixa
        free(novo_pedido);
        return true; // Pedido descartado, mas continue lendo
    }

    // 7. Checagem de Adiamento (Postergation)
    ...
    if (tempo_total_estimado > LIMITE_POSTERGAR_PEDIDO || tempo_total_estimado > timer_global) {
        ...
        return true; // Pedido adiado, mas continue lendo
    }

    // 8. Alocação no Caixa (Se passou em tudo)
...
bool existemPedidosAtivos(Restaurante *res) {
    for (int i = 0; i < 5; i++) {
        if (res->locais[i].fila_espera.quantidade > 0) return true; // Fila única
...
// Conta todos os pedidos que estão *depois* do caixa
int contarPedidosEmProducao(Restaurante *res) {
    int total = 0;
    
    // Pedidos na fila do Separador (Única + Prioridade)
...
void liberarRestaurante(Restaurante *res) {
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        liberarListaPedidos(&res->locais[i].fila_espera); // Fila única
